...
const txt = (text: string, name = "pgdas_202701.txt") =>
...
test("QA: competência do PGDAS prevalece sobre a data de geração", async () => {
  const r = await parseFileRaw(
    txt(
      "PGDAS-D\nData de geração: 15/02/2027\nCNPJ: 12.345.678/0001-95\nPeríodo de Apuração: 01/2027\nRBT12: 1.200.000,00\nReceita bruta do período de apuração (RPA): 100.000,00",
    ),
  );
  assert.ok("competencia" in r && "dados" in r && r.dados);
  assert.equal(r.competencia, "2027-01");
  assert.equal(
    "faturamento" in r.dados ? r.dados.faturamento : undefined,
    100000,
  );
  assert.equal("rbt12" in r.dados ? r.dados.rbt12 : undefined, 1200000);
});
...
test("QA: folha com competência no cabeçalho não importa o mês como salário", async () => {
  ...
  assert.ok("tipo" in r && "dados" in r && r.dados);
  assert.equal(r.tipo, "folha");
  assert.equal("folha" in r.dados ? r.dados.folha : undefined, 12000);
  assert.equal("proLabore" in r.dados ? r.dados.proLabore : undefined, 4000);
});
test("QA: não soma duas declarações PGDAS da mesma competência", () => {
  ...
  assert.throws(
    () =>
      aggregateDocuments(
        [a, { ...a, id: "b", dados: { faturamento: 110000, rbt12: 1200000 } }],
        "2027-01",
      ),
    /PGDAS/,
  );
...
test("QA: cálculo comercial conferido contra memória numérica independente", () => {
  ...
  assert.ok(Math.abs(r.regimes.tradicional.mensal - 8825) < 0.0001);
  assert.ok(Math.abs(r.regimes.hibrido.mensal - 13617.125) < 0.0001);
  assert.ok(Math.abs(r.regimes.presumido.mensal - 26576) < 0.0001);
});
test("QA: cálculos de serviços III, IV e V contra valores conferidos", () => {
  ...
  assert.ok(Math.abs(a.regimes.tradicional.mensal - 13030) < 0.0001);
  assert.ok(Math.abs(a.regimes.presumido.mensal - 22176) < 0.0001);
  const b = computeRegimes({ ...d, anexoServicosForcado: "IV" });
  assert.ok(Math.abs(b.regimes.tradicional.mensal - 14125) < 0.0001);
  assert.ok(Math.abs(b.regimes.hibrido.mensal - 17827.45) < 0.0001);
  const c = computeRegimes({ ...d, anexoServicosForcado: "V" });
  assert.ok(Math.abs(c.regimes.tradicional.mensal - 19075) < 0.0001);
});
test("QA: dados inválidos não produzem recomendação automática", () => {
  assert.equal(
    computeRegimes({ ...exampleState(), cestaPct: 90, reduzido60Pct: 90 })
      .melhor,
    null,
  );
