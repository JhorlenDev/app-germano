> Versão moderna preservada. A página principal agora é o HTML original; consulte README.md.
...
4. Execute `docker compose -f compose.moderno.yaml up -d --build`. A aplicação fica em `127.0.0.1:3087`.
...
docker compose -f compose.moderno.yaml exec -e ADMIN_NAME -e ADMIN_EMAIL -e ADMIN_PASSWORD app npx tsx server/create-user.ts
...
```bash
docker compose -f compose.moderno.yaml exec -T db pg_restore -U diagnostico -d diagnostico --no-owner < backups/SEU_BACKUP.dump
```
